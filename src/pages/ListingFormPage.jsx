...
function Section({ label, children, sx }) {
  return (
    <Box sx={{ mb: { xs: 2.5, sm: 4.5 }, ...sx }}>
      ...
      <Box sx={{ borderBottom: '1px solid #EEEEEE', mb: { xs: 1.5, sm: 2.5 } }} />
      {children}
    </Box>
  );
}
...
function FieldLabel({ children, required }) {
  return (
    <Typography sx={{ fontSize: 12, fontWeight: 700, color: '#374151', mb: { xs: 0.5, sm: 0.75 }, letterSpacing: '0.02em' }}>
        ...
        return (
          <Box
            ...
            sx={{
              ...
              px: 1.5,
              py: hasIcons ? { xs: 1, sm: 1.5 } : { xs: 0.9, sm: 1.25 },
              textAlign: 'center',
              display: 'flex',
...
export default function ListingFormPage() {
  ...
  const [fields, setFields] = useState({
    ...
    // Location — country/state hold ISO codes (for cascading lookups),
    // city holds the plain city name.
    latitude: '', longitude: '', street: '', city: '', state: '', country: 'IN', pincode: '',
  });
  ...
  /* ── country-state-city option lists (cascading) ── */
  const countryOptions = useMemo(() => Country.getAllCountries(), []);
  const stateOptions = useMemo(
    () => (fields.country ? State.getStatesOfCountry(fields.country) : []),
    [fields.country]
  );
  const cityOptions = useMemo(
    () => (fields.country && fields.state ? City.getCitiesOfState(fields.country, fields.state) : []),
    [fields.country, fields.state]
  );

  const selectedCountry = useMemo(
    () => countryOptions.find((c) => c.isoCode === fields.country) || null,
    [countryOptions, fields.country]
  );
  const selectedState = useMemo(
    () => stateOptions.find((s) => s.isoCode === fields.state) || null,
    [stateOptions, fields.state]
  );
  const selectedCity = useMemo(
    () => cityOptions.find((c) => c.name === fields.city) || null,
  ...
  const handleStateChange = (_, option) => {
    setFields((prev) => ({
      ...prev,
      state: option?.isoCode || '',
      city: '',
    }));
  };
  ...
  const handleCityChange = (_, option) => {
    setFields((prev) => ({
      ...prev,
      city: option?.name || '',
      // Auto-fill coordinates from the city record when available, but
      // don't clobber a value the user has already typed in manually.
      latitude: option?.latitude && !prev.latitude ? option.latitude : prev.latitude,
      longitude: option?.longitude && !prev.longitude ? option.longitude : prev.longitude,
    }));
  };
  ...
  const doSubmit = async () => {
    ...
    // Resolve human-readable names from the selected ISO codes.
    const stateName = selectedState?.name || fields.state;
    const countryName = selectedCountry?.name || fields.country;
  ...
  const progress = useMemo(() => {
    const hasLocation = (!!fields.latitude && !!fields.longitude) || !!fields.city || !!fields.street;
    const base = [
      !!fields.title.trim(),
      images.length > 0,
      hasLocation,
    ...
    if (listingType === 'DEAL') extra.push(!!fields.dealPrice || !!fields.price, !!fields.validFrom || !!fields.validUntil);
      ...
      <Container maxWidth="md" sx={{ px: { xs: 2, sm: 3 }, py: { xs: 2.5, sm: 4 } }}>
        <Box component="form" onSubmit={handleSubmit}>
          ...
          <Section label="Images">
            <Paper
              ...
              sx={{
                ...
                borderRadius: 2.5, p: { xs: 2.5, sm: 4 }, textAlign: 'center',
          ...
          <Section label={`${meta.short} Info`}>
            <Box sx={{ display: 'flex', flexDirection: 'column', gap: { xs: 1.75, sm: 2.5 } }}>
              <Box>
            ...
            <Grid container spacing={{ xs: 1.5, sm: 2.5 }}>
              <Grid item xs={12} sm={4}>
                ...
                <Autocomplete
                  ...
                  renderInput={(params) => (
                    <TextField {...params} placeholder="Select country" sx={inputSx} />
                  )}
                />
              </Grid>
              <Grid item xs={6} sm={4}>
                ...
                <Autocomplete
                  ...
                  renderInput={(params) => (
                    <TextField {...params} placeholder="Select state" sx={inputSx} />
                  )}
                />
              </Grid>
              <Grid item xs={6} sm={4}>
                ...
                <Autocomplete
                  ...
                  renderInput={(params) => (
                    <TextField {...params} placeholder="Select city" sx={inputSx} />
                  )}
                />
              </Grid>
              <Grid item xs={12}>
